...
func TestOpenAPINullabilitySemanticsMatrix(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantLibTypes:      []string{"boolean"},
			wantNullable:      boolPtrForNullabilityTest(true),
			wantDeclaredTypes: []string{"boolean"},
			wantCandidate:     true,
		},
		{
			...
			wantLibTypes:      []string{"boolean", "null"},
			wantDeclaredTypes: []string{"boolean", "null"},
			wantCandidate:     true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if doc.Version() != tt.version {
				t.Fatalf("version=%q want=%q", doc.Version(), tt.version)
			}
			...
			if got := normalizedTypes(libTypes); !reflect.DeepEqual(got, tt.wantLibTypes) {
				t.Fatalf("libopenapi schema.Type=%v want=%v", got, tt.wantLibTypes)
			}
			if !sameOptionalBool(nullable, tt.wantNullable) {
				t.Fatalf("libopenapi schema.Nullable=%v want=%v", optionalBoolString(nullable), optionalBoolString(tt.wantNullable))
			}
			...
			t.Logf("version=%s libopenapi.Type=%v Nullable=%s DeclaredTypes=%v Ambiguous=%t Required=%t candidate=%t",
				doc.Version(), normalizedTypes(libTypes), optionalBoolString(nullable), property.DeclaredTypes, property.Ambiguous, property.Required, candidatePresent)
		})
	}
}

func responsePropertyLibrarySemantics(doc *Document, propertyName string) ([]string, *bool, error) {
	op, _, err := matchOperation(doc, http.MethodPost, "/probe")
	if err != nil {
		return nil, nil, err
	}
	response, _, err := selectResponse(op.Responses, http.StatusOK)
	if err != nil {
		return nil, nil, err
	}
	media, _, err := selectMediaType(response.Content, "application/json")
	if err != nil {
		return nil, nil, err
	}
	...
	if root == nil || root.Properties == nil {
		return nil, nil, fmt.Errorf("response root schema has no properties")
	}
	for name, proxy := range root.Properties.FromOldest() {
		...
		if schema == nil {
			return nil, nil, fmt.Errorf("property %s returned nil schema", propertyName)
		}
